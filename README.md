...
# Расписание: будни, 05:00-08:00 MSK — прошлый торговый день до открытия торгов
...
Слотов четыре (05:00, 06:00, 07:00, 08:00), потому что время публикации
истории на ISS не гарантировано. Обертка идемпотентна: отчет за день
делается и отправляется ровно один раз, остальные слоты выходят молча.
Если данных еще нет, `detector.py` возвращает код 3 — это не ошибка,
уведомление просто не отправляется до следующей попытки.
